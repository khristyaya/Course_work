...
# Обов'язково зберігаємо колонку Date при об'єднанні
...
# Видалення Date, якщо більше не потрібен
df_raw = df_raw.drop(columns=["Date"])

# Перетворення колонок у категоріальні типи
df_raw["Season"] = df_raw["Season"].astype("category")

# Label Encoding для HomeTeam та AwayTeam
...
# Збереження енкодерів для майбутнього використання
...
# Початкова модель Random Forest
